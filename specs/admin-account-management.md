...
>    + a "Se déconnecter" button + a hint about the `npm run reset-admin` recovery path. Polls
>    `/users/me/email-status` on mount AND on every route change so the banner appears anywhere
>    the operator navigates and disappears the moment they log back in with the new address (the
>    login flow updates `lastLoginAt` to a more recent timestamp).
...
| `components/EmailVerifyBanner.js` | — | C | **Follow-up #7 (2026-06-02).** Persistent anti-lockout banner mounted in `AppShell` between the `AppBar` and the routed pages. Polls `/api/users/me/email-status` on mount + every route change; renders an MUI Alert (warning) iff `mustVerifyNewEmail === true`. Shows the new email + a "Se déconnecter" button + a one-liner pointing operators at `npm run reset-admin` for SSH recovery. Self-contained: returns `null` when there's nothing to warn about, so wiring is a single `<EmailVerifyBanner />`. |
