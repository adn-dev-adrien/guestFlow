...
4. Authorize request: scopes `https://www.googleapis.com/auth/calendar.events` + `https://www.googleapis.com/auth/calendar.readonly` + `openid email`, `access_type=offline`, `prompt=consent` (guarantees a refresh token on every connect), CSRF `state` = `crypto.randomBytes(16).toString('hex')` stored in `req.session.googleOAuthState` (Qonto pattern).
