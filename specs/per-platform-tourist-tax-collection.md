...
| `models/financeModel.js` | `financeModel.js` | T | `getTouristTaxExtraction` SQL `WHERE` now reads `r.platform = 'direct' OR EXISTS (SELECT 1 FROM ical_sources s WHERE s.propertyId = r.propertyId AND (lower(s.platformKey) = lower(r.platform) OR lower(s.platformLabel) = lower(r.platform)) AND s.collectsTouristTax = 0)` (2026-06-19: added the `platformLabel` arm). **2026-06-21:** the month guard accepts up to **and including the current month** (`month > currentMonth` is rejected, was `>=`) — PR #253 unblocked the client picker but the server still 400'd the month in progress with « Seuls les mois déjà passés sont autorisés. »; the message is now « Les mois futurs ne sont pas autorisés. » Covered by `tourist-tax-collection-coverage.unit.test.js` (current-month accepted + future rejected). |
