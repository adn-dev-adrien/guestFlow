...
12. The **cot resource is not listed** with the supplements: as in the current popup, a « Lit(s) bébé souhaité(s) ? (gratuit, selon disponibilité) » stepper appears only when Bébés ≥ 1 (max = number of babies) and is sent as the devis `babyBeds` field — not as a resource line. *Amended 2026-09-25: the widget recognises it by the API's `isBabyBed` flag (`specs/translation-catalogue.md` rule 22), not by matching « Lit bébé » on the payload. It matched the name until the translation catalogue started serving « Baby bed » to an English page, and the cot then appeared among the extras for every English visitor, babies or none. The name test survives only as the fallback for a GuestFlow older than the flag — the plugin updates itself from its own manifest, so it can run ahead of the server.*
