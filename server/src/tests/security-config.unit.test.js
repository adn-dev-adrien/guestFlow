...
//
//   | NODE_ENV    | HTTPS_EDGE    | CSP        | HSTS  | upgrade-insecure-requests | secure cookie |
//   |-------------|---------------|------------|-------|---------------------------|---------------|
//   | development | (any)         | OFF        | OFF   | n/a                       | OFF           |
//   | production  | (unset/false) | ON (SPA)   | OFF   | OFF                       | OFF           |
//   | production  | true          | ON (SPA)   | ON    | ON                        | ON            |
//
// HTTPS_EDGE = "an HTTPS edge (Caddy) sits in front of the app" — NOT "the app terminates TLS"
// (it never does anymore). Changing the rules MUST update both the table above and the cases below.
...
test('shouldEnforceHttps: defaults to false (HTTPS_EDGE missing means no HTTPS edge guarantee)', () => {
  assert.equal(shouldEnforceHttps({}), false);
  assert.equal(shouldEnforceHttps({ HTTPS_EDGE: 'false' }), false);
  assert.equal(shouldEnforceHttps({ HTTPS_EDGE: '1' }), false);
  // NODE_ENV does NOT influence it on purpose — production deploy without a proxy stays usable.
  assert.equal(shouldEnforceHttps({ NODE_ENV: 'production' }), false);
});

test('shouldEnforceHttps: true only when HTTPS_EDGE is the literal "true"', () => {
  assert.equal(shouldEnforceHttps({ HTTPS_EDGE: 'true' }), true);
  assert.equal(shouldEnforceHttps({ HTTPS_EDGE: 'TRUE' }), true);
});
...
test('helmet: development → CSP off, HSTS off (allow plain-HTTP dev session)', () => {
  const opts = buildHelmetOptions({ isProduction: false, httpsEdge: false });
  assert.equal(opts.contentSecurityPolicy, false);
  assert.equal(opts.strictTransportSecurity, false);
});

test('helmet: production + HTTPS_EDGE=false → CSP on (SPA-tuned), HSTS OFF, no upgrade-insecure-requests', () => {
  const opts = buildHelmetOptions({ isProduction: true, httpsEdge: false });
  ...
  assert.equal('upgradeInsecureRequests' in csp.directives, false, 'no upgrade-insecure-requests when no HTTPS edge');
...
test('helmet: production + HTTPS_EDGE=true → CSP on, HSTS on, upgrade-insecure-requests added', () => {
  const opts = buildHelmetOptions({ isProduction: true, httpsEdge: true });
...
test('helmet: development + HTTPS_EDGE=true → still no CSP (dev convenience wins over CSP)', () => {
  // A developer playing with the prod CSP locally needs to opt in via NODE_ENV=production too.
  const opts = buildHelmetOptions({ isProduction: false, httpsEdge: true });
  assert.equal(opts.contentSecurityPolicy, false);
  // HSTS however is unrelated to NODE_ENV — if an HTTPS edge is declared, respect it.
  assert.equal(opts.strictTransportSecurity, true);
});
...
test('helmet: cross-origin resource policy stays cross-origin in every mode', () => {
  ...
  for (const isProduction of [false, true]) {
    for (const httpsEdge of [false, true]) {
      const opts = buildHelmetOptions({ isProduction, httpsEdge });
...
test('cookie: secure flag tracks httpsEdge exactly (no NODE_ENV dependency)', () => {
  assert.equal(buildSessionCookieOptions({ httpsEdge: false }).secure, false);
  assert.equal(buildSessionCookieOptions({ httpsEdge: true }).secure, true);
});

test('cookie: httpOnly + sameSite + maxAge are constants (security baseline)', () => {
  for (const httpsEdge of [false, true]) {
    const opts = buildSessionCookieOptions({ httpsEdge });
