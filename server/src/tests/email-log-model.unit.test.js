...
// ---- listPending × automatic sending ----
// specs/settings-rationalization.md rule 17b: a template's own mode is the only switch. An « auto »
// template leaves with the 08:00 pass, so the review queue only ever proposes the « manual » ones —
// on every anchor.

test('listPending: an « auto » template is never proposed, on any anchor', () => {
  const { db, model } = fresh();
  seedFixture(db);
  ...
  db.prepare("INSERT INTO email_templates (id, stableKey, name, subject, body, dayOffset, sendMode, enabled, anchor) VALUES (60, NULL, 'AutoDevis', 'S', 'B', 0, 'auto', 1, 'validUntil')").run();
  const rows = model.listPending({ today: '2026-06-10' });
  assert.ok(!rows.some((r) => [11, 50, 60].includes(r.templateId)), 'no auto template in the queue');
  assert.ok(rows.some((r) => r.templateId === 10), 'the manual one still surfaces');
});
