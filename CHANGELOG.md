...
### Summary
- Majeure : le site refuse toute réservation sans CGV acceptées — publier la v1 des CGV et passer le plugin WordPress en 1.8.0 le jour même de l'installation.
- Les CGV s'écrivent dans Paramètres → Conditions générales ; chaque réservation du site en garde la preuve, visible sur la fiche.
- Booking, Airbnb et Abritel peuvent se synchroniser en iCal direct, sans Lodgify ; les fermetures partent aussi dans les flux exportés.
- Réglages réorganisés : une page par sujet, une page Plateformes, la page logement en onglets, « Clients » dans le menu principal.
- Le bouton « Acompte » d'un logement vaut désormais partout : désactivé, le solde porte tout (un acompte déjà encaissé reste intact).
- L'anti-spam du site compte par visiteur et non plus pour tout le site ; renseigner « Proxys de confiance » dans le plugin.

### Added
- **Establishment closures in the public iCal feed** (spec `ical-export-closures.md`, 2026-09-20). A property's `.ics` export now carries the closures that apply to it — its own plus the global ones — so Booking, Airbnb, Gîtes de France and GreenGo stop selling a period declared closed instead of having the incoming booking silently dropped by the sync guard. Closures that are already over are left out; the event reads `Fermeture — <label>`, carries no guest data, and keeps the closure's stored dates (start inclusive, end exclusive). +13 server tests.
...
- **A property's « Acompte » switch now governs the whole deposit** (spec `property-deposit-switch.md`, 2026-09-20). It only applied to the website: a fiche created in the admin still split deposit and balance. Now, when it is off, the property has no deposit anywhere — direct bookings, platforms and website — and the balance carries the full amount. A deposit **already collected** is never rewritten: its split and dates survive every recalculation, so the accounting export stays in step. +8 server tests.
- **Property page — the « Acompte & Solde » tile is split in two** (spec `property-deposit-switch.md`). « Acompte » opens on its switch and only reveals « % acompte » and « Acompte (jours après réservation) » when it is on; « Paiement & Caution » keeps « Solde (jours avant) », « Annulation (jours après échéance du solde) » and « Caution par défaut », always visible since they also apply to a stay paid in one go. +3 client tests.
...
- **`properties.publicDepositEnabled` → `properties.depositEnabled`** (spec `property-deposit-switch.md`, 2026-09-20). Idempotent `ALTER TABLE … RENAME COLUMN` in `database.js`, mirrored in `schema.sql`; every stored value is preserved. The field is renamed in `/api/properties` (client and server ship in the same archive); `/public/v1` is untouched.
- **Behaviour change: the default becomes « no deposit ».** The column was already 0 everywhere, but it only governed the website; it now governs the whole deposit. Consequence: a reservation carrying an **uncollected** deposit on a property with the switch off will see that deposit move into the balance at the next recalculation of its fiche (an edit, for instance). No migration rewrites amounts already quoted to a guest.
...
### Summary
- Qonto se configure et se répare depuis Réglages → Paiements : identifiants, test de connexion, et un bandeau qui ne ment plus.
- L'assurance annulation se souscrit automatiquement chez Neat quand l'acompte d'une réservation directe assurée est encaissé.
- Taxe de séjour : le calcul suit désormais celui de l'office du tourisme, et chaque ligne affiche le montant HT qui l'a produite.
- La taxe se fige quand le client la paie, et non plus quand le séjour est passé : un séjour ancien jamais encaissé redevient corrigeable.
- Les règles des specs sont mesurées : une règle ajoutée sans test fait désormais échouer la PR.

### Added
- **Qonto is configured and repaired from the interface** (spec `qonto-settings-in-app.md`). The application credentials — environment, client id, client secret, sandbox staging token, webhook secret, public site origin — are entered in Réglages → Paiements, stored encrypted, and take effect on the next call: no file to edit, no restart. A field left empty keeps reading its environment variable, so an installation set up through `.env.local` is untouched until someone opens the form. The client id is deliberately readable — it is a public identifier, and comparing it with the one the Qonto portal shows is the first step of every diagnosis.
- **« Tester la connexion » says what is broken, and what repairs it.** The button makes a real authenticated call to Qonto and classifies the outcome in French: credentials rejected, authorisation to renew, link provider not enabled, Qonto unreachable — each with the one action that fixes it. A rejected secret (`invalid_client`) and a lapsed authorisation (`invalid_grant`) both arrive as an HTTP 401; one is a field to retype and the other a button to press, and they are no longer confused.
- **The Qonto badge tells the truth, and failures surface.** « Connecté » is now claimed only after a real call has succeeded, never on the mere presence of a stored token. Every failed Qonto call — public booking tunnel, scheduled poll, webhook, hand-made link — records its code, its message and its origin, and shows up in Réglages → Paiements. Between 2026-08-19 and 2026-09-06 the payment tunnel was dead while the page read « Connecté ✓ »; that failure mode is now impossible.
- **Automatic Neat subscription for the cancellation insurance** (spec `neat-cancellation-insurance-subscription.md`). When the deposit of an insured direct reservation is collected, GuestFlow subscribes the policy through Neat's API (unique external id — never a duplicate), with automatic retries and a push notification on failure. A new « Assurance annulation (Neat) » settings card holds the encrypted credentials, the test/production environment, the channel/contract/payment-mode choices and the contract field mapping; the reservation's insurance card gains a Neat status badge with « Réessayer » and « Résilier ». The guest price becomes « Neat premium + margin %, rounded up to the euro » when a margin is configured (falling back to the last known price, then to the manual Options tariff). Cancelling a policy stays a deliberate manual gesture.
- **A spec rule without a test is now visible — and soon impossible** (spec `spec-rule-coverage.md`). `node scripts/check-spec-coverage.mjs` finally answers « which rules have no test? » by reading the convention already in place in the comments (584 « rule N » citations), imposing nothing new. A CI job turns the measurement into a barrier: on a pull request, a rule **added** to a spec must be cited by a test or the check fails. No inherited debt is demanded — only the diff is examined. Measured starting point: 2 501 rules, 474 covered (19 %).
- **The accounting core now knows which of its rules are proven** (phase 3 of `spec-rule-coverage.md`). Five payment and accounting specs go from **8 covered rules out of 125 to 88**: each test now names the rule it proves, and the rules that have none are visible instead of invisible. Three rules deciding what a **réception** account may see and write about a stay's money had no test at all; they have one now. Repository-wide coverage: 19 % → 23 %.
- **Arrival payment now knows which of its rules are proven.** Five stay-and-collection specs go from **18 covered rules out of 111 to 80**. Four rules genuinely had no test and now do: the trace left in the history when a single payment is collected, the refusal opposed to a **réception** account trying to collect it, the contribution snapshot taken on each flipped instalment (and the fact that a failed capture does not cost the collection), and the stay notes left out of the group. Repository-wide coverage: 23 % → 25 %.
- **The money core now knows which of its rules are proven.** Five specs — refunds, adjustable complements, schedule and cancellation, accounting export, platform transfer delay — go from **95 covered rules out of 195 to 193** (6 of them declared « sans test » with their reason). Eight rules genuinely had no test and now do: the refusal opposed to a **réception** account on the deadlines card, the refusal to dun a **platform** guest (the most embarrassing email the app could send), the server-side validation of adjusted complement amounts, the recap shown before cancelling a stay, the read-only state of a cancelled stay, the contract of the five money email templates, the exclusion of cancelled stays from the financial tracking, and a late refund's attachment to the stay's month. Three specs were corrected along the way — an email token announced but never built, and five rules describing templates replaced since. Repository-wide coverage: 25 % → 29 %.

### Fixed
- **Tourist tax: the calculation now follows the tourist office's.** The base starts again from the gross paid to the platform, less the invoiced options, the cleaning and linen included in the tariff, and the extra-guest supplement; infants no longer count among the occupants who divide the night. The amount charged to the guest is now the one that will actually be remitted.
- **A tax line justifies itself.** The reservation and the « Suivi taxe de séjour » page show the « Montant du séjour HT » that produced the displayed amount — copyable as-is into the commune's form. Until now the amount was frozen while the base kept being recomputed: a line could announce 24,73 €/occupant/night next to 21,00 €.
- **The tax freezes when the guest pays it**, or when the line is ticked « Déclarée » — no longer when the stay is over. A finished stay that was never collected can therefore finally be corrected.
